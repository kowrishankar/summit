...
export default function AddInvoiceScreen({
  ...
  const { user } = useAuth();
  const { addInvoice, updateInvoice, addCategory, categories, invoices, currentBusiness } = useApp();
    ...
    try {
      if (isPdf) {
        ...
        try {
          ...
          if (pdfBase64) {
            const data = await extractFromPdfBase64(pdfBase64, fileName ?? 'invoice.pdf');
            setExtracted(data);
            setStep('review');
            return;
          }
        ...
        Alert.alert(
          'PDF not supported',
          'Could not process this PDF. Try the web app or take a photo of the invoice instead.'
        );
        setStep('preview');
      } else {
        const assets =
  ...
  const save = async () => {
    ...
    try {
      ...
      // Reset state so next time user opens Add they see the choose step, not previous invoice
      setStep('choose');
      setExtracted(null);
      setCategoryId(null);
      setFileName('');
